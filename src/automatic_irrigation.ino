...
void setup() {
  ...
  timer.setInterval(1000L, relay_);
}
...
void hygro_calibrate() {
  ...
  // if (hygroval <= MOISTURE_LOW && !relayState) {
  //   water_start();
  // } 
  // else if (hygroval > MOISTURE_HIGH && relayState) {
  //   water_stop();
  // }
  delay(100);
}
...
void relay_(){
  ...
  else if (hygroval >= MOISTURE_HIGH) {
    digitalWrite(RELAYPIN, LOW);
    relayState = false;
    Serial.println("Water stopped");
