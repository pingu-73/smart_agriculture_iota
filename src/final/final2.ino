...
const float calibrationFactor = 2200; // Calibration factor for the sensor
...
void relayControl() {
  int hygro_analog = analogRead(HYGRO_PIN);
  // int x =  (100 - ((hygro_analog / 4095.00) * 100));
  if (hygroval < 40) {
    ...
    if(total_ml > 3750){
      ...
      Serial.println("Total quantity exceeded 3750ml, stopping water");
      total_ml = 0;
      flow_frequency = 0;
      Blynk.virtualWrite(V2, total_ml);
    }
